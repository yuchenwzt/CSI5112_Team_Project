...
class OrderDetailState extends State<OrderDetailPage>
            ...
            child: Column(
              children: [
                ListTile(
                  ...
                  title: Text('Order ' +
                      widget.order.salesOrder.order_id.substring(0, 6) +
                      ' ... Details'),
                  subtitle:
                ...
                Padding(
                  ...
                  child: Text(
                    ...
                    style: TextStyle(
                        color: widget.order.salesOrder.status == 'Processing'
                            ? Colors.red
                            : Colors.green),
                  ),
                ),
                ButtonBar(
                  ...
                  children: [
                    widget.user.isMerchant
                        ? TextButton(
                            onPressed:
                                widget.order.salesOrder.status == 'Processing'
                                    ? () {
  ...
  void initState() {
    ...
    if (widget.user.isMerchant) {
      _presenter.loadAddress(HttpRequest('Get',
          'ShippingAddress/by_user?user_id=${widget.user.merchant_id}', {}));
    }
    _presenter2.loadProducts(HttpRequest('Get',
        'Products?product_id=${widget.order.salesOrder.product_id}', {}));
  }
